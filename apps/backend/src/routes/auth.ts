...
import { hashPassword, verifyPassword, signToken, setAuthCookie, clearAuthCookie, authMiddleware } from "../auth";
...
authRoute.post("/auth/login", async (c) => {
  ...
  try { body = await c.req.json(); } catch { return c.json({ error: "Invalid email or password" }, 401); }
  ...
  const token = await signToken({ userId: user.id, role: user.role as "worker" | "manager", name: user.name });
  setAuthCookie(c, token);
